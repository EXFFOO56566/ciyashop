...
jQuery(document).ready(function() {
    ...
    jQuery('.ciyashop-native-app').on('click',function(){
        ...
        if(native_app == 'android'){
            jQuery('.android-frm-field-group').show();
            jQuery('.ios-frm-field-group').hide();
            if((!jQuery('.activate-code-btn').length > 0) && (!envato_setup_params.purchased_android)){
                jQuery('.envato-setup-actions').append('<input type="submit" class="activate-code-btn button button-large button-next button-primary" value="Activate"/>');

            } else if(envato_setup_params.purchased_android) {
                jQuery('.activate-code-btn').remove();

            }
        } else {
            jQuery('.android-frm-field-group').hide();
            jQuery('.ios-frm-field-group').show();
            if((!jQuery('.activate-code-btn').length  > 0) && (!envato_setup_params.purchased_ios)){
                jQuery('.envato-setup-actions').append('<input type="submit" class="activate-code-btn button button-large button-next button-primary" value="Activate"/>');

            } else if(envato_setup_params.purchased_ios) {
                jQuery('.activate-code-btn').remove();
